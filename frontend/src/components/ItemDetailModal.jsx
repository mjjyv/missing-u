...
const ClaimRequestItem = ({ claim, onVerify, onOpenChat }) => {
      ...
      {claim.status === 'ACCEPTED' && (
        <>
          ...
          <button
            ...
            className="w-full mt-3 py-4 bg-blue-600 text-white font-bold text-lg rounded-2xl shadow-xl hover:bg-blue-700 transition"
          >
            💬 CHAT VỚI {claim.full_name.toUpperCase()}
          </button>
        </>
      )}
    </div>
...
export default function ItemDetailModal({ item, onClose, onUpdateList }) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        // 3. Người xem: claim của mình
        if (user && !isOwner) {
          try {
            const myRes = await axiosClient.get(`/claims/my-claim/${item.id}`);
            if (isMounted && myRes.data.data) {
              setMyClaim(myRes.data.data);
            }
          } catch (err) {
            if (isMounted) setMyClaim(null);
          }
        }
      } catch (err) {
  ...
  // Mở chat (owner)
  const handleOpenChatOwner = (claim) => {
    setChatPartnerName(claim.full_name || 'Người dùng');
    setActiveChatClaimId(claim.id);
  };

  // Mở chat (claimer)
  const handleOpenChatClaimer = () => {
    if (myClaim && myClaim.status === 'ACCEPTED') {
      setChatPartnerName(item.author_name || 'Chủ bài đăng');
      setActiveChatClaimId(myClaim.id);
    }
  };
                  ...
                  <div className="space-y-4">
                    {incomingClaims.map(claim => (
                      <ClaimRequestItem
                        ...
                        onOpenChat={handleOpenChatOwner}
                      />
                    ))}
                  </div>
                )}
                ...
                {myClaim ? (
                  <div className={`p-6 rounded-3xl border-2 shadow-lg ${
                    ...
                    {myClaim.status === 'ACCEPTED' && (
                      <button
                        onClick={handleOpenChatClaimer}
                        className="w-full py-4 bg-blue-600 text-white font-bold text-lg rounded-2xl shadow-xl hover:bg-blue-700 transition"
                      >
                        💬 BẮT ĐẦU TRÒ CHUYỆN VỚI CHỦ BÀI
                      </button>
                    )}
                  </div>
                ) : (
